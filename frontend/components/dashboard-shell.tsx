...
function NavLink({
  ...
  return (
    ...
    >
      {active && (
        <span
          className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-[62%] rounded-full bg-gradient-to-b from-cyan-200 via-white to-violet-300 shadow-[0_0_12px_rgba(255,255,255,0.6)]"
          aria-hidden
        />
      )}
      <span
      ...
      {active && (
        <span className="hidden xl:block h-1.5 w-1.5 rounded-full bg-emerald-300 shadow-[0_0_8px_rgba(52,211,153,0.9)] shrink-0" aria-hidden />
      )}
    </Link>
  );
}
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = (
    <>
      <NavLink href="/dashboard" label="Tableau de bord" icon={LayoutDashboard} onNavigate={() => setMobileOpen(false)} />
      <NavLink href="/dashboard/evenements" label="Mes événements" icon={Folder} onNavigate={() => setMobileOpen(false)} />
      <NavLink href="/dashboard/abonnements" label="Abonnements" icon={CreditCard} onNavigate={() => setMobileOpen(false)} />
      {user?.is_superuser && (
        <NavLink href="/admin/config" label="Thème (admin)" icon={Settings} onNavigate={() => setMobileOpen(false)} />
      )}
    </>
  );

  /** Largeur sidebar (lg:pl-* doit rester synchronisé) */
    ...
    <div className="min-h-svh flex flex-col bg-background">
      ...
      >
        ...
        <div className="relative shrink-0 px-5 pt-6 pb-5">
          <div className="flex items-start justify-between gap-2">
            <BrandMark linkHref="/dashboard" variant="light" size="md" />
          </div>
          <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-white/45 mt-4">Workspace</p>
          <p className="text-sm text-white/80 mt-1 leading-snug font-medium">Espace organisateur</p>
        </div>
        ...
        <div className="relative shrink-0 p-4 pt-3 border-t border-white/[0.07] bg-gradient-to-t from-black/15 to-transparent">
          <div className="rounded-2xl border border-white/15 bg-white/[0.08] backdrop-blur-xl p-3.5 flex items-center gap-3 shadow-[0_12px_40px_-16px_rgba(0,0,0,0.5)] ring-1 ring-white/10">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate">{user?.nom ?? "Organisateur"}</p>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <div
            className={cn(
              "absolute left-0 top-0 bottom-0 w-[min(88vw,20rem)] bg-sidebar text-sidebar-foreground shadow-2xl flex flex-col border-r border-white/10 overflow-hidden",
            )}
          >
            <div
              ...
              style={{
                backgroundImage:
                  "radial-gradient(ellipse 100% 60% at 0% 0%, rgba(99,102,241,0.18) 0%, transparent 55%), radial-gradient(ellipse 80% 50% at 100% 100%, rgba(16,185,129,0.1) 0%, transparent 45%)",
              }}
            />
            <div className="relative p-4 flex items-center justify-between border-b border-white/10">
              <BrandMark linkHref="/dashboard" variant="light" size="sm" />
              <button
                type="button"
        ...
        <header className="sticky top-0 z-40 shrink-0 border-b border-border/50 bg-background/75 backdrop-blur-xl supports-[backdrop-filter]:bg-background/65">
          <div className="max-w-7xl mx-auto w-full px-4 lg:px-8 py-2.5">
            <div className="flex flex-col gap-3 w-full">
              {/* Mobile : menu + marque | heure + profil */}
              <div className="flex items-center justify-between gap-3 lg:hidden">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <div className="min-w-0 truncate">
                    <BrandMark linkHref="/dashboard" size="sm" />
                  </div>
                </div>
              ...
              {/* Titre / description / actions à gauche — heure + profil à droite (desktop) ; sous la barre mobile si contenu */}
              <div
                className={cn(
