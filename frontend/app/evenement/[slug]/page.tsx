...
export default function GuestAlbumPage({ params }: { params: { slug: string } }) {
  ...
  const albumSlug = (searchParams.get("album") || "public").trim() || "public";
  ...
  const basePath = `/api/evenements/${slug}/albums/${albumSlug}/album/`;

  const init = useQuery({
    queryKey: ["album", slug, albumSlug, "init"],
  ...
  const singleQ = useInfiniteQuery({
    queryKey: ["album", slug, albumSlug, "single"],
  ...
  const pubQ = useInfiniteQuery({
    queryKey: ["album", slug, albumSlug, "section", "public"],
    enabled: init.isSuccess && split,
    initialPageParam: 1,
  ...
  const privQ = useInfiniteQuery({
    queryKey: ["album", slug, albumSlug, "section", "private"],
    enabled: init.isSuccess && split,
    initialPageParam: 1,
    queryFn: async ({ pageParam }) =>
  ...
  function invalidateAlbum() {
    void qc.invalidateQueries({ queryKey: ["album", slug, albumSlug] });
  }
  ...
  async function downloadZipAfterPayment(transactionId: string) {
    try {
      ...
      a.href = url;
      a.download = `photos-${slug}-${albumSlug}.zip`;
